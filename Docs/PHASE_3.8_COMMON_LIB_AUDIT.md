...
The highest-traffic SQL-executing functions from the narrative section above are the strongest candidates for a future targeted-extraction phase, following the `LegacyQueryAdapter` shape already established by `limit_subcategory()` and `open_or_closed()`. This phase does not pick which one goes first — that's a future phase's own brainstorm.

The 3 escape-discard-bug functions (`convert_to_ba`, `convert_to_pro`, `remove_sensitive_data`) are a special case, not an unqualified addition to the above: they also appear in Bucket 1 as zero-caller, zero-test dead-code candidates. They are extraction-worthy only if the empirical dead-code verification Bucket 1 recommends finds them still live; if that verification confirms they're truly dead, Bucket 1's delete path applies instead, which makes the narrative section's in-place-fix suggestion for these three moot.
