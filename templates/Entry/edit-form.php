...
<head>
    ...
    <style>
        ...
        button { background: #1976d2; color: white; padding: 0.75em 1.5em; border: none; cursor: pointer; margin-right: 0.5em; }
...
<body>
    ...
    <?php if ($entryWindow !== 1): ?>
        <div class="alert alert-danger">
            Entry editing window is currently closed. You can no longer modify this entry.
        </div>
    <?php endif; ?>

    <form method="POST" action="/entries/<?= e((string) $entry->id()->value()) ?>">
        <input type="hidden" name="id" value="<?= e((string) $entry->id()->value()) ?>">
        ...
        <div class="form-group">
            ...
            <input type="text" id="brewName" name="brewName" required maxlength="255"
                   value="<?= e($_POST['brewName'] ?? $entry->name()) ?>">
            ...
            <select id="brewCategorySort" name="brewCategorySort" required>
                ...
                <?php foreach ($styles ?? [] as $style): ?>
                    <option value="<?= e($style['brewCategorySort']) ?>"
                            <?= ($_POST['brewCategorySort'] ?? $entry->style()->group()) === $style['brewCategorySort'] ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" id="brewSubCategory" name="brewSubCategory" required maxlength="10"
                   value="<?= e($_POST['brewSubCategory'] ?? $entry->style()->num()) ?>">
        ...
        <div class="form-group">
            ...
            <a href="/entries/my" style="text-decoration: none; color: #666;">Cancel</a>
        </div>
    </form>

    <div style="margin-top: 2em; padding-top: 2em; border-top: 1px solid #ccc;">
        <h3>Danger Zone</h3>
        <p>Once deleted, this entry cannot be recovered.</p>
        <form method="POST" action="/entries/<?= e((string) $entry->id()->value()) ?>" style="display: inline;">
