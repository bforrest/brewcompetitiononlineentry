...
<div class="row mb-3">
    ...
    <div class="col-xs-12 col-sm-9 col-lg-10"><input class="form-control<?= isset($form->fieldErrors['password']) ? ' is-invalid' : '' ?>" id="password-entry" name="password" type="password" placeholder="Password" required><?php if (isset($form->fieldErrors['password'])): ?><div class="help-block invalid-feedback text-danger"><?= e($form->fieldErrors['password']) ?></div><?php endif; ?></div>
</div>
...
<div class="row mb-3">
    ...
    <div class="col-lg-10 col-md-9 col-sm-8 col-xs-12"><input class="form-control password-field<?= isset($form->fieldErrors['password-confirm']) ? ' is-invalid' : '' ?>" name="password-confirm" id="password-confirm" type="password" required><?php if (isset($form->fieldErrors['password-confirm'])): ?><div class="help-block invalid-feedback mt-1 text-danger"><?= e($form->fieldErrors['password-confirm']) ?></div><?php endif; ?><div id="password-error" class="help-block mt-1 text-danger"></div></div>
</div>
<div class="mb-3 row"><label for="security" class="col-xs-12 col-sm-3 col-lg-2 col-form-label text-teal"><i class="fa fa-star me-1"></i><strong>Security Question</strong></label><div class="col-xs-12 col-sm-9 col-lg-10"><?php foreach ($questions as $question): ?><div class="form-check"><input class="form-check-input<?= isset($form->fieldErrors['userQuestion']) ? ' is-invalid' : '' ?>" type="radio" name="userQuestion" value="<?= e($question) ?>"<?= $selectedQuestion === $question ? ' checked' : '' ?>><label class="form-check-label"><?= e($question) ?></label></div><?php endforeach; ?><div class="help-block">Choose one. This question will be used to verify your identity should you forget your password.</div><?php if (isset($form->fieldErrors['userQuestion'])): ?><div class="help-block invalid-feedback d-block text-danger"><?= e($form->fieldErrors['userQuestion']) ?></div><?php endif; ?></div></div>
<div class="mb-3 row"><label for="userQuestionAnswer" class="col-xs-12 col-sm-3 col-lg-2 col-form-label text-teal"><i class="fa fa-star me-1"></i><strong>Security Question Answer</strong></label><div class="col-xs-12 col-sm-9 col-lg-10"><input class="form-control<?= isset($form->fieldErrors['userQuestionAnswer']) ? ' is-invalid' : '' ?>" name="userQuestionAnswer" id="userQuestionAnswer" type="text" value="<?= e((string) ($form->values['userQuestionAnswer'] ?? '')) ?>" required><div class="help-block">Make your security answer something only you will easily remember!</div><?php if (isset($form->fieldErrors['userQuestionAnswer'])): ?><div class="help-block invalid-feedback text-danger"><?= e($form->fieldErrors['userQuestionAnswer']) ?></div><?php endif; ?></div></div>
