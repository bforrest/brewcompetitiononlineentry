...
| **Flaky Tests** | 0 known — `RegistrationContainerWiringTest` and `ConnectionBootstrapIntegrationTest` order-dependent flake fixed 2026-07-22: changed `require_once (CONFIG.'config.php')` to `require (CONFIG.'config.php')` in `paths.php` so config.php re-executes on each call from the Connection factory when `$GLOBALS['connection']` is unset. `MysqliDb.php` (class definition) stays `require_once`. No production impact: normal web requests require paths.php exactly once per process, so the distinction never arises outside PHPUnit's shared process. | ✓ Fixed | ✓ |
...
**Status:** GREEN (0 errors, 0 failures)

| Check | Pass | Notes |
|-------|------|-------|
| All integration tests pass | ✓ | 24 files, 144 tests — real run 2026-07-22 (post-merge to `slim`): `Tests: 144, Assertions: 304, Warnings: 51, Deprecations: 5, Skipped: 28`. The 2 previous errors (`RegistrationContainerWiringTest` and `ConnectionBootstrapIntegrationTest`) were fixed by changing `require_once (CONFIG.'config.php')` to `require (CONFIG.'config.php')` in `paths.php`. 0 failures (the 2 `TotalFeesTest` failures seen mid-merge were stray `e2e-*@example.com` rows from this session's own earlier Playwright runs against this DB, cleaned up — see `sweepOrphanTestData()` gap below). |
...
**Known Issues:**
- ~~**Order-dependent flake:** `RegistrationContainerWiringTest` and `ConnectionBootstrapIntegrationTest` — fixed 2026-07-22 (`require_once → require` for `site/config.php` in `paths.php`).~~
...
**Known Flakes:** 0 — `RegistrationContainerWiringTest` and `ConnectionBootstrapIntegrationTest` order-dependent flake fixed 2026-07-22
...
- ~~**`RegistrationContainerWiringTest` order-dependent flake:** fixed 2026-07-22 (`require_once → require` in `paths.php`).~~
...
- [x] Fix `RegistrationContainerWiringTest`'s and `ConnectionBootstrapIntegrationTest`'s order-dependent `$GLOBALS['connection']` bootstrap flake — fixed 2026-07-22: `require_once → require` for `site/config.php` in `paths.php`
