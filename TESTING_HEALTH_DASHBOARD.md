...
| **Flaky Tests** | 2 known, same root cause — `RegistrationContainerWiringTest::test_registration_service_resolves` and `ConnectionBootstrapIntegrationTest::test_connection_class_bootstraps_from_scratch` (both order-dependent; both pass reliably in isolation, both fail with `mysqli connection not initialized in $GLOBALS['connection']` when run inside the full Integration suite). Root cause confirmed while merging this branch: `require_once`'s "already included" tracking for `site/config.php` is process-wide, not per-test — once ANY earlier Integration test causes it to load, a later test that deliberately unsets `$GLOBALS['connection']` and expects `Connection::class` to re-bootstrap it via a fresh `require ROOT.'paths.php'` finds that `require_once (CONFIG.'config.php')` inside `paths.php` silently no-ops. **Confirmed NOT a production concern** — a real HTTP request gets a genuinely fresh `require_once` table every time (PHP's per-request lifecycle resets it), so this only manifests in PHPUnit's single continuous process. A `#[RunTestsInSeparateProcesses]` fix was attempted and reverted: it does isolate the process correctly, but PHPUnit's separate-process runner escalates `paths.php`'s pre-existing, already-tolerated session-ini_set warnings (see `phpunit.xml`'s "failOnWarning intentionally NOT set" note) into hard errors, trading one false failure for another. Left as a known, order-dependent test-suite artifact. | ⚠ Not new | ⚠ |
...
| Check | Pass | Notes |
|-------|------|-------|
| All integration tests pass | ⚠ | 24 files, 144 tests — real run 2026-07-22 (post-merge to `slim`): `Tests: 144, Assertions: 304, Errors: 2, Warnings: 51, Deprecations: 5, Skipped: 28`. The 2 errors are `RegistrationContainerWiringTest::test_registration_service_resolves` and `ConnectionBootstrapIntegrationTest::test_connection_class_bootstraps_from_scratch` (both `RuntimeException: mysqli connection not initialized in $GLOBALS['connection']`) — reproduced consistently when run as part of the full suite, **both pass reliably in isolation**. Confirmed same root cause for both: `require_once`'s process-wide (not per-test) tracking for `site/config.php`; confirmed not a production concern (see Summary row above for the full explanation and the separate-process fix attempt that was reverted). 0 failures (the 2 `TotalFeesTest` failures seen mid-merge were stray `e2e-*@example.com` rows from this session's own earlier Playwright runs against this DB, cleaned up — see `sweepOrphanTestData()` gap below). |
...
**Known Issues:**
- **Order-dependent flake (pre-existing, since Task 8/9; root cause confirmed while merging this branch):** `RegistrationContainerWiringTest::test_registration_service_resolves` and `ConnectionBootstrapIntegrationTest::test_connection_class_bootstraps_from_scratch` both fail with `mysqli connection not initialized in $GLOBALS['connection']` when run as part of the full Integration suite; both pass standalone. Root cause: `require_once`'s "already included" tracking for `site/config.php` is process-wide — once any earlier Integration test causes it to load, a later test that unsets `$GLOBALS['connection']` and expects a fresh `require ROOT.'paths.php'` to re-bootstrap it finds `config.php`'s body silently doesn't re-execute. Confirmed NOT a production issue (real HTTP requests get a fresh `require_once` table per request). A `#[RunTestsInSeparateProcesses]` fix correctly isolates the process but trades this failure for a different one (PHPUnit's separate-process runner escalates `paths.php`'s existing, already-tolerated session-ini_set warnings into hard errors) — reverted rather than shipped.
...
**Known Flakes:** 2, same root cause — `RegistrationContainerWiringTest::test_registration_service_resolves` and `ConnectionBootstrapIntegrationTest::test_connection_class_bootstraps_from_scratch` (order-dependent, see Tier 2 above)
...
- [ ] Fix `RegistrationContainerWiringTest`'s and `ConnectionBootstrapIntegrationTest`'s order-dependent `$GLOBALS['connection']` bootstrap flake (passes standalone, fails in full suite; root cause is `require_once`'s process-wide tracking for `site/config.php` — see Tier 2 above for the full trace and the reverted separate-process fix attempt)
