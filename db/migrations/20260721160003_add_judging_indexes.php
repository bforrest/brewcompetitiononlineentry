    ...
    public function change(): void
    {
        ...
        $judging_scores
            ->addIndex(['scoreTable', 'eid'], [
                ...
                'unique' => true,
                'comment' => 'Unique constraint: one score per entry per table'
            ])
            ->addIndex(['scoreTable'], [
            ...
            ->addIndex(['eid'], [
                ...
                'unique' => false,
                'comment' => 'For queries like: find all tables where this entry was scored'
            ])
            ->addIndex(['scoreTable', 'version'], [
                'name' => 'idx_judging_scores_table_version',
                'unique' => false,
                'comment' => 'For optimistic locking: WHERE scoreTable=? AND version=?'
            ])
            ->update();
        ...
        $judging_flights
            ->addIndex(['flightTable', 'flightRound', 'flightNumber'], [
                ...
                'unique' => false,
                'comment' => 'For maintaining flight queue order at a table'
            ])
            ->addIndex(['flightEntryID'], [
                'name' => 'idx_judging_flights_entry',
                'unique' => false,
                'comment' => 'For queries: find all flights for an entry'
            ])
            ->update();
        ...
        $judging_assignments
            ->addIndex(['assignTable', 'assignRound'], [
                ...
                'unique' => false,
                'comment' => 'For finding judges/stewards assigned to a table in a round'
            ])
            ->addIndex(['assignJudge'], [
                'name' => 'idx_judging_assignments_judge',
                'unique' => false,
                'comment' => 'For finding all assignments for a judge'
            ])
            ->update();
    }
}
